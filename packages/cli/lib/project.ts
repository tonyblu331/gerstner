...
const JS_ENTRY_NAMES = ['main.tsx', 'main.ts', 'main.jsx', 'main.js', 'index.tsx', 'index.ts', 'index.jsx', 'index.js']
...
export async function detectCssEntry(cwd: string): Promise<string | undefined> {
  ...
  const matches = await walkForMatches(cwd, (entryPath) => /(?:^|\/)src\/(?:index|styles)\.css$/u.test(normalize(entryPath)))
  return matches[0]
}
...
export async function detectJsEntry(cwd: string, cssEntry: string): Promise<string | undefined> {
  ...
  const matches = await walkForMatches(cwd, (entryPath) => /(?:^|\/)(?:main|index)\.[cm]?[jt]sx?$/u.test(normalize(entryPath)))
  return matches[0]
}
...
async function walkForMatches(root: string, predicate: (entryPath: string) => boolean): Promise<string[]> {
